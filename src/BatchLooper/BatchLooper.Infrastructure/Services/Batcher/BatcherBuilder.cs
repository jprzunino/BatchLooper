...
{
    [DebuggerDisplay("BatcherBuilder: {_configurator?.BatchCount} x {_configurator?.BatchSize} | Parallelism = {_configurator?.MaxDegreeOfParallelism} | MaxTries = {_configurator?.MaxTries} | WaitTime = {_configurator?.WaitTime} (s)")]
    public class BatcherBuilder<TEntity> : IBatcherBuilder<TEntity>
    {
        #region Variables

        private protected IBatcherConfiguration<TEntity> _configurator;
        private protected IBatchExecutionEngine<TEntity> _engine;
        private protected IBatchProgressPrinterConsole<TEntity> _printer;
        ...
        public BatcherBuilder([NotNull] IBatcherConfiguration<TEntity> configuration)
            : this() => _recursive.WithConfigurations(configuration ?? throw new ArgumentNullException(nameof(configuration)));

        public BatcherBuilder([NotNull] IEnumerable<TEntity> source)
            : this(new BatcherConfiguration<TEntity>(source)) { }
        ...
        void IBatcherBuilder<TEntity>.WithConfigurations([NotNull] IBatcherConfiguration<TEntity> configuration)
        {
            ...
            _printer = new BatchProgressPrinterConsole<TEntity>(_semaphore, _configurator);
            _statistics = new BatchStatistics<TEntity>(_printer, new());
            _engine = new BatchExecutionEngine<TEntity>(_semaphore, _configurator, _printer, _statistics);
        }
