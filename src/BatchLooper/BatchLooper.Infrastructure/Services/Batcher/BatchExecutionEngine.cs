...
{
    /*
     * Atention must sync this with 'BatchExecutionEngineSerilog'.
     * Atenção refetir ajustes no 'BatchExecutionEngineSerilog'.
     */
    [DebuggerDisplay("BatchExecutionEngine: Status = {Status} | CurrentBatch = {_currentBatch} | LastError = {LastErrorMessage,nq}")]
    public class BatchExecutionEngine<TEntity> : IBatchExecutionEngine<TEntity>
    {
        #region Variables
        ...
        public BatchExecutionEngine([NotNull] ISemaphorePattern semaphore, [NotNull] IBatcherConfiguration<TEntity> batcherConfiguration, [NotNull] IBatchProgressPrinterConsole<TEntity> printer, [NotNull] IBatchStatistics<TEntity> stats)
        {
        ...
        {
            if (_config.PrintProgressManual && _currentManager is not null)
            {
                _printer.HideCursor();
                _printer.PrintProgressManual(_currentManager.TotalProgress, _stats.TotalDuration);
                _stats.PrintSummaryManual(Status);
                _printer.ShowCursor();
            }
        }
        ...
        private Task SetUpPipelineAsync([NotNull] Func<IEnumerable<TEntity>, Task> action, CancellationToken cancellationToken)
        {
            ...
            _actionHandle = new ActionBlock<IEnumerable<TEntity>>(async items => await action(items).ConfigureAwait(_defaultConfigureAwait), _config.ExecutionOptions);
        ...
        private async Task LinkPipelineAsync([NotNull] Func<Task> action)
        {
            ...
            await action().ConfigureAwait(_defaultConfigureAwait);
        }
           ...
           await LinkPipelineAsync(async () =>
            {
                try
                {
                    ...
                    _printer.PrintConsole($"Start Process: {_config.BatchCount:N0} x {_config.BatchSize:N0} ({_config.SourceCount:N0}) | Parallelism: {_config.MaxDegreeOfParallelism} | Tries: {_config.MaxTries} | WaitBeteewnTries: {_config.WaitTime} (s).", inDebugToo: true);
                    _printer.HideCursor();
        ...
        private async Task BatchHandleCollectionAsync([NotNull] BatchManagerModel<TEntity> batchManager, [NotNull] Func<IEnumerable<TEntity>, Task> action, Func<bool>? skipHandle = null)
        {
            ...
            await _stats.CollectDurationAsync(async timestamp =>
            {
                try
                {
                    ...
                    var exceptions = await RetryExtension.RetryAsync(
                        action: async () => await action(batchManager.BatchInfo?.Collection.Span.ToArray().Select(s => s.Data!).ToImmutableArray() ?? []).ConfigureAwait(_defaultConfigureAwait),
                        actionInError: (ex, tries) =>
                        {
                            Interlocked.Exchange(ref _status, (int)BatcherBuilderStatus.WithErrors);
                            _printer.PrintDebug($"[Tries: {tries}] => {ex.GetAggregatedMessages()}");
        ...
        private async Task BatchHandleCollectionAsync([NotNull] BatchManagerModel<TEntity> batchManager, [NotNull] Func<BatchInfoModel<TEntity>, Task> action, Func<bool>? skipHandle = null)
        {
            ...
            await _stats.CollectDurationAsync(async timestamp =>
            {
                try
                {
                    ...
                    var exceptions = await RetryExtension.RetryAsync(
                        action: async () => await action(batchManager.BatchInfo!).ConfigureAwait(_defaultConfigureAwait),
                        actionInError: (ex, tries) =>
                        {
                            Interlocked.Exchange(ref _status, (int)BatcherBuilderStatus.WithErrors);
                            _printer.PrintDebug($"[Tries: {tries}] => {ex.GetAggregatedMessages()}");
        ...
        private void BatchHandleSequential([NotNull] BatchManagerModel<TEntity> batchManager, [NotNull] Action<TEntity> action, Func<bool>? skipHandle = null)
        {
            #region Batch Handle
        ...
        private void BatchHandleParallel([NotNull] BatchManagerModel<TEntity> batchManager, [NotNull] Action<TEntity> action, Func<bool>? skipHandle = null)
        {
            ...
            _stats.CollectDuration((timestamp) =>
            {
                batchManager.BatchInfo!.Collection
        ...
        private void Release()
        {
            ...
            _currentManager?.BatchInfo?.Collection.Span.Clear();
